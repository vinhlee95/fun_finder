...
def save_available_slots_for_day(available_slots: DailyAvailableSlot):
...
def save_available_slots_for_period(available_slots: list[DailyAvailableSlot]):
  for daily_slots in available_slots:
    save_available_slots_for_day(daily_slots)


def main():
  try:
      # Fetch available slots from Smash API
    save_available_slots_for_period(fetch_smash_olari_availability(DEFAULT_PERIOD))
